...
- [x] **Audio** — **complete 2026-09-01**, playtested on desktop and tablet. `PLAN-SOUND.md` is authoritative. The Augmentinel Amiga sample set is converted and accepted (`public/sounds/*.m4a`, ten files, 4.1 MB), regenerated by `utils/convert-sounds.sh` and refereed by `utils/sound-snr.py`; no code plays anything yet and `settings.soundVolume` still has no effect. The headlines: **no music during gameplay** (the original's near-silence is kept deliberately — `music` plays on the menu screen, plus the *touch* pause screen only, since a desktop pause is an interruption rather than a destination), **no transfer sound** (it existed to cover a 3 s blank screen we don't have), **hyperspace gets a synthesized ~1 s cue** (the sample is a five-note motif that survives neither cutting nor speeding up), **`seen` pulses rather than loops**, and the positional sounds are genuinely positional through Three.js `PositionalAudio`. This bullet's original wish-list — create, error and ambient-hum sounds — is left open there: no sample covers any of them.
...
- [ ] **`settings.rotationInterval` is dead and the menu lies about it.** Found 2026-09-01 while
      checking the watcher cadence for `PLAN-SOUND.md`. It is declared in `settings.svelte.ts`
      (default 10), rendered by `MainMenu.svelte` as `Rotation interval: 10s` and adjustable 2–16s,
      and **read by nothing**. Worse than inert: it advertises control over a cadence that is
      hardcoded — `TURN_PERIOD_TICKS = 48` in `world/objects/watcher.ts`, one step every 12 s before
      the per-completion speedup — so a player who changes it is told the game changed and it did
      not. The second inert setting found this way, after `soundVolume`, so it is worth a sweep for
      a third.

      **The intent is a difficulty setting**, and it should be wired rather than removed. Augmentinel
      offers five steps from **15 s (very easy)** down to **6 s (very hard)**, with **12 s** as the
      default — which is exactly our hardcoded `TURN_PERIOD_TICKS`, so the current behaviour already
      *is* Augmentinel's default and nothing needs re-tuning to adopt the scale. Our menu's current
      2–16 s continuous range is the wrong shape for that: it should become five named steps, not a
      slider. The two middle values were not read off and want confirming in Augmentinel before
      implementing (15 / 12 / ? / ? / 6). **Out of scope for the sound work** — recorded here so the
      intent is not lost, since the entry has looked like a bug rather than an unfinished feature.
