...
- [x] **Win / Lose screens.** `WinScreen.svelte` / `LoseScreen.svelte`, mounted for WON/LOST. Each has its own `<svelte:window onkeydown>` calling `completeWon()`/`completeLost()` directly — since those already guard on current phase, a keypress just fires the same completion the 2s timeout would have, and `App.svelte`'s existing effect-cleanup cancels the now-redundant timeout automatically (no new wiring needed there). WinScreen reads `settings.levelId` + `game.energy` before `completeWon()` mutates them (the hyperspace's `spendEnergy(3)` already ran before `triggerWon()`, so `game.energy` at display time already IS the jump amount).
