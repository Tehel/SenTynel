...
**Planning a hop the purse cannot finish (2026-08-25, `PLAN-BOT2.md` postscript 16, landscape 7398).** Reported from watching with the fix already in it — *"if we don't have enough to build a boulder+synthoid, just build the synthoid"* — and that is precisely right. 7398's opening needs a three-boulder tower, leaving 1 energy; the bot transfers up, reclaims the body it left (+3), and with **four** in hand plans a hop costing **five**. It lays the boulder, is left with two, and stops for the rest of the run: nothing clears `this.plan`, the harvest has nothing in reach, and rung 6's hyperspace needs 6, so it cannot even leave. One transfer in the whole run, at every epoch tried. **The obvious gate is not the fix** — the walk prices only the *next* action (landscape 16's lesson, which `remainingHopCost` exists for), but pricing the whole hop would merely swap one stall for another, since at four energy every hop carrying a boulder is unaffordable and refusing them all stalls just as hard. A *cheaper hop* was the only answer. Nor is it postscript 8's rejected change, which dropped the climb boulder on **geometry** and bought an opening at the ascent's expense; this fires on **solvency** and is invisible whenever the bot has money. **912 → 919/1000**, +8 −1, mean jump unchanged, `watchdog-stalled` unchanged; v1 untouched via an optional `budget`. **Two wrong versions were shipped and caught first, both on landscape 6313 — reported from watching its very first move**, and both worth knowing because each looked like obviously correct arithmetic. *Shortening* the pile to what the purse can buy makes it gain nothing at all: `bouldersToOutrank` returns the **smallest** pile that lifts the feet above where the body stands, so every lesser count tops out at or under our own height — 6313 spent eight energy raising a tower exactly level with itself, stepped sideways onto it and arrived broke in a pocket. *Dropping* the pile instead turns a climb into a **fall**: that same destination lies two levels below, so the bot built a body down there, transferred into a tile with no hop-field entry, and proposed a hyperspace the rules refused fifty-nine times. Only the discretionary climb-while-you-walk boulder may go. **And the rejected shorten-it version measures 924 — five better — and is rejected anyway**, on postscript 8's precedent: its gain is `never-reached-assault-position` 49 → 32 bought with `watchdog-stalled` 14 → 25, so it is not climbing better but spending less per hop and moving faster against the clock, on a mechanism that is incoherent by construction. A rule wrong on every individual landscape and right in aggregate is a measurement saying something *else* is mispriced — probably that piles are over-bought generally, which wants `chooseDestination` to pick a **different** destination when it cannot fund one, rather than build a pile that does nothing. **The pattern across the last five reports** (7632, 9950, 7398) is worth stating plainly: all three were *permanent* states rather than bad play, all found by watching an unattended run, and none visible in a win-rate total — 7632 occurs nowhere in the verdict block, 9950 wins at the harness default epoch, 7398 read as one loss among ninety. The sweep is how you tell whether a change helped; it is not how you find out what is wrong.

**A square being absorbed still counted as occupied (2026-08-25, `RULES-FIDELITY.md` A5, `PLAN-BOT2.md` postscript 17).** Reported from watching landscape 6313's opening — *"why does the bot build in the pit when the cell next to the start is perfect? Probably because that cell starts with a tree on it, absorbed as the first action, and is thus improper for building during the second it takes to disappear"* — and that was exactly it. `engine/scene.ts` held **two disagreeing views of a stack**: `objectsAt` (and through it `topObjectAt`, the surface rule, every LOS test and the bot's whole world snapshot) filters `absorbedTime === null`, while `canPlaceAt` and `addObjectToScene` re-derived the stack themselves and did not — so for the 1–2 s an absorb animation plays, the square still reported *occupied* to the one rule that decides whether anything may be built there. Two things make it a defect rather than a judgement call: **a cosmetic setting changed what the rules allow** (`Settings → Game → Animation` picks `squash` at 1 s or `fade`/`dissolve` at 2 s, so a display preference set whether the block cost one action slot or two), and it is **the same bug as the 2026-08-24 raycast fix one site further on** — that pass gave absorbing objects `skipRaycast` on exactly this reasoning and missed `canPlaceAt`. Both now filter; `addObjectToScene`'s "don't double-place during the fade-out" comment was wrong, since a stack whose top boulder is vanishing should place the next one where that boulder no longer is. **919 → 921/1000**, mean jump 35.3 → 35.4, every bucket flat or better, nothing lost. **It also silenced the go-home rung**: 7632 now takes a different route and `goHome` fires zero times there and zero times across a traced scan of 6000-6149, so postscript 14's fixture no longer exercises what it was built for — the assertion is removed with the reason written where it stood, and the rung's only guard is now its unit test. The rung stays (its conditions are still reachable), but this is the C9 shape again, and **two of the last four fixes have now removed the very situation an earlier fix was built for** — the other face of "the sweep is not how you find out what is wrong".
