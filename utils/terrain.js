...
// -------------------------- Utils --------------------------
const range = (v) => Array(v)
    .fill(0)
    .map((_, i) => i);
const sum = (a) => a.reduce((s, v) => s + v, 0);
const max = (a) => a.reduce((m, v) => Math.max(m, v), -Infinity);
const decAsHex = (v) => parseInt(v.toString(10), 16);
export const lpad = (s, c, n) => c.repeat(n - s.length) + s;
// -------------------------- RNG --------------------------
let ull;
let rngUsage;
...
const rngn = (n) => range(n).map(rng256);
...
function generateCodePart(system) {
    if (system === 'BBC/C64')
        return rngBCD();
    // PC, ST and Amiga consume 3 additional rands
    if (system === 'PC/ST' || system === 'Amiga')
        range(3).forEach(_ => rng256());
    const b = rngBCD();
    const a = rngBCD();
...
function despikeMidval(v1, v2, v3) {
    if (v2 === v3)
        return v2;
    if (v2 > v3) {
        if (v2 <= v1)
...
function despike(map, dim, axis) {
    const newMap = map.slice();
    if (axis == 'z') {
        for (let z = 0; z < dim; z++) {
            // initialize a slightly longer row copy
            const v = Array(dim + 3);
            for (let x = 0; x < dim + 3; x++)
                v[x] = newMap[z * dim + (x % dim)];
            // despike it
            for (let x = dim; x > 0; x--)
                v[x] = despikeMidval(v[x - 1], v[x], v[x + 1]);
        ...
        for (let x = 0; x < dim; x++) {
            // initialize a slightly longer row copy
            const v = Array(dim + 3);
            for (let z = 0; z < dim + 3; z++)
                v[z] = newMap[(z % dim) * dim + x];
            // despike it
            for (let z = dim; z > 0; z--)
                v[z] = despikeMidval(v[z - 1], v[z], v[z + 1]);
            // copy back updated values
            for (let z = 0; z < dim; z++)
                newMap[z * dim + x] = v[z];
        }
...
function scaleAndOffset(val, scale) {
...
function tileShape(fl, bl, br, fr) {
    /*
        0 = flat
...
function addTileShapes(map, dim) {
...
// -------------------------- Object placement --------------------------
export var GameObjType;
(function (GameObjType) {
    GameObjType[GameObjType["SYNTHOID"] = 0] = "SYNTHOID";
    GameObjType[GameObjType["SENTRY"] = 1] = "SENTRY";
    GameObjType[GameObjType["TREE"] = 2] = "TREE";
    GameObjType[GameObjType["BOULDER"] = 3] = "BOULDER";
    GameObjType[GameObjType["MEANIE"] = 4] = "MEANIE";
    GameObjType[GameObjType["SENTINEL"] = 5] = "SENTINEL";
    GameObjType[GameObjType["PEDESTAL"] = 6] = "PEDESTAL";
})(GameObjType || (GameObjType = {}));
function printGameObject(object) {
    ...
    if (object.rot !== null)
        s += ` rot=${object.rot.toString(16)} (${rotdeg}${rotdir})`;
    if (object.timer !== null)
        s += ` next=${object.timer /*:02X*/}`;
    return s;
}
...
function calcNumSentries(levelId) {
    ...
    if (levelHex >= 0x0100 || max_sentries > 7)
        max_sentries = 7;
...
function createObjectAt(type, x, y, z) {
...
export function generateLevel(levelId, options) {
    const dim = MAP_SIZE;
